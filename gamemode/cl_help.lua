...
function GM:CreateHelpMenu()
	...
	CCP.HelpMenu.Plugins = vgui.Create("DButton", CCP.HelpMenu);
	CCP.HelpMenu.Plugins:SetPos(10, y);
	CCP.HelpMenu.Plugins:SetSize(120, 20);
	CCP.HelpMenu.Plugins:SetText("Plugins");
	CCP.HelpMenu.Plugins:PerformLayout();
	function CCP.HelpMenu.Plugins:DoClick()
		CCP.HelpMenu.Selected = self;

		local text = "Combine Control Plugin Loading\n\n"
		for i, v in pairs(GAMEMODE.Stored) do
			text = text .. "Plugin: " .. i .. "\nPlugin Author: " .. v.Author .. "\nPlugin Description: " .. v.Description .. "\n\n\n";
		end
		CCP.HelpMenu.Content:SetText(text);
		CCP.HelpMenu.Content:InvalidateLayout( true );
		CCP.HelpMenu.ContentPane:PerformLayout();
	end
